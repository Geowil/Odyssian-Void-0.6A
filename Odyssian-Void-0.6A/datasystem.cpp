...
Database db_ds;  //Global matrix defines
...
int dataSystem::getSSM(){
	return ships.at(0).sSM;
}

int dataSystem::getSAP(){
	return ships.at(0).sAP;
}

int dataSystem::getSHS(){
	return ships.at(0).sHS;
}

int dataSystem::getSCPU(){
	return ships.at(0).sCPU;
}

int dataSystem::getSRAM(){
	return ships.at(0).sRAM;
}

int dataSystem::getSEngine(){
	return ships.at(0).sEng;
}

int dataSystem::getSPS(){
	return ships.at(0).sPS;
}

int dataSystem::getSSPS(){
	return ships.at(0).sSPS;
}

int dataSystem::getSTS(){
	return ships.at(0).sWTS;
}

int dataSystem::getSWFG(){
	return ships.at(0).sWFG;
}

int dataSystem::getSLSlots(){
	return ships.at(0).lSlots;
}

int dataSystem::getSMSlots(){
	return ships.at(0).mSlots;
}

int dataSystem::getSHSlots(){
	return ships.at(0).hSlots;
}
...
int dataSystem::getSXCost(){
	return ships.at(0).sXCost;
}

int dataSystem::getSRCost(){
	return ships.at(0).sRCost;
}

int dataSystem::getSDiCost(){
	return ships.at(0).sDiCost;
}

int dataSystem::getSDCost(){
	return ships.at(0).sDCost;
}

int dataSystem::getSUCost(){
	return ships.at(0).sUCost;
}

int dataSystem::getSPCost(){
...
string dataSystem::getWEName()
{
	return weapons.at(0).wEName;
}

float dataSystem::getWEAmount()
{
	return weapons.at(0).wEAmount;
}

int dataSystem::getWEDuration()
{
	return weapons.at(0).wEDuration;
}
...
void dataSystem::rData(string table, string operation, int ID)
{
	...
	else if (table == "Skill")
	{
		...
		db_ds.getSkResults(&bErrors);
		db_ds.returnSkResults(skills);
	}

	else if (table == "Requirement_Data")
	{
		db_ds.tableTAccess("Requirement_Data");
		db_ds.createStatement(ID, operation);
...
//Skill Functions
string dataSystem::getSkName()
{
	return skills.at(0).skName;
}

string dataSystem::getSkDesc()
{
	return skills.at(0).skDesc;
}

string dataSystem::getSkEffect()
{
	return skills.at(0).skEffect;
}

string dataSystem::getSkType()
{
	return skills.at(0).skType;
}

string dataSystem::getSkGroup()
{
	return skills.at(0).skGroup;
}

int dataSystem::getSkTTime()
{
	return skills.at(0).skTTime;
}

float dataSystem::getSkTDiff()
{
	return skills.at(0).skTDiff;
}

int dataSystem::getSkXCost()
{
	return skills.at(0).skXCost;
}

int dataSystem::getSkCXPCost()
{
	return skills.at(0).skCXPCost;
}
...
//Ship Equipment Functions
int dataSystem::getSETTime()
{
	return sEquipment.at(0).seTTime;
}

int dataSystem::getSEMTargs()
{
	return sEquipment.at(0).seMTargs;
}

float dataSystem::getSEPReq()
{
	return sEquipment.at(0).sePReq;
}

float dataSystem::getSECPReq()
{
	return sEquipment.at(0).seCReq;
}

float dataSystem::getSERReq()
{
	return sEquipment.at(0).seRReq;
}

float dataSystem::getSECMax()
{
	return sEquipment.at(0).seCMax;
}

float dataSystem::getSESRRate()
{
	return sEquipmnet.at(0).seSRRate;
}

float dataSystem::getSERRate()
{
	return sEquipment.at(0).seRRate;
}

float dataSystem::getSEMAP()
{
	return sEquipment.at(0).seMAP;
}

float dataSystem::getSERPCon()
{
	return sEquipment.at(0).seRPCon;
}

float dataSystem::getSEMThrust()
{
	return sEquipment.at(0).seMThrust;
}

float dataSystem::getSENSMax()
{
	return sEquipment.at(0).seNSMax;
}

float dataSystem::getSENPCon()
{
	return sEquipment.at(0).seNPCon;
}

float dataSystem::getSETRCWgt()
{
	return sEquipment.at(0).seTRCWgt;
}

float dataSystem::getSECDrain()
{
	return sEquipment.at(0).seCDrain;
}

int dataSystem::getSEDTime()
{
	return sEquipment.at(0).seDTime;
}

float dataSystem::getSESMax()
{
	return sEquipment.at(0).seSMax;
}

float dataSystem::getSEIPCon()
{
	return sEquipment.at(0).seIPCon;
}

float dataSystem::getSEPMax()
{
	return sEquipment.at(0).sePMax;
}

float dataSystem::getSEMHP()
{
	return sEquipment.at(0).seMHP;
}

float dataSystem::getSEMSP()
{
	return sEquipment.at(0).seMSP;
}
...
bool dataSystem::lData(Player& mPlayer, Game* mGame)
{
	...
	if (!bErrors)
	{
		...
		db_ds.createStatement(0, "select from");  		//Player Ship
	...
	if (getFlag("PSWSpreads") == 1)
	{
		...
		if (!bErrors)
		{
			...
			cout << "Done" << endl << endl;  			//Open Save DB
			db_ds.openSave(&bErrors);
		}
	...
	if (!bErrors) //Only continue if save database could be opened
	{
		...
		for (i = 0;; i++)
		{
			if (i == 0)
			{
				...
				plShips.at(plShips.size() - 1).lSlots = mPlayer.ship.getLSlots();
				plShips.at(plShips.size() - 1).mSlots = mPlayer.ship.getMSlots();
				plShips.at(plShips.size() - 1).hSlots = mPlayer.ship.getHSlots();
				plShips.at(plShips.size() - 1).sULSlots = mPlayer.ship.getULSlots();
				plShips.at(plShips.size() - 1).sUMSlots = mPlayer.ship.getUMSlots();
				plShips.at(plShips.size() - 1).sUHSlots = mPlayer.ship.getUHSlots();
				plShips.at(plShips.size() - 1).sMWSpreads = mPlayer.ship.getSWSpreads();
				plShips.at(plShips.size() - 1).sEngine = mPlayer.ship.sEngine.getID();
				plShips.at(plShips.size() - 1).sPSystem = mPlayer.ship.sPSystem.getID();
				plShips.at(plShips.size() - 1).sCPU = mPlayer.ship.sCPU.getID();
				plShips.at(plShips.size() - 1).sRAM = mPlayer.ship.sRAM.getID();
				plShips.at(plShips.size() - 1).sSPSystem = mPlayer.ship.sSPSystem.getID();
				plShips.at(plShips.size() - 1).sSMatrix = mPlayer.ship.sSMatrix.getID();
				plShips.at(plShips.size() - 1).sWFGen = mPlayer.ship.sWFGen.getID();
				plShips.at(plShips.size() - 1).sAPlating = mPlayer.ship.sAPlating.getID();
				plShips.at(plShips.size() - 1).sHSStruct = mPlayer.ship.sHSStruct.getID();
				plShips.at(plShips.size() - 1).sWTSystem = mPlayer.ship.sWTSystem.getID();
				...
				if (mPlayer.ship.sCargo.size() > 0)
				{
					...
					for (i2 = 0; i2 < mPlayer.ship.sCargo.size(); i2++)
					{
						if (mPlayer.ship.sCargo.at(i2).isShip())
						{
							...
							plShips.at(plShips.size() - 1).lSlots = tempInv.at(i2).sShip.getLSlots();
							plShips.at(plShips.size() - 1).mSlots = tempInv.at(i2).sShip.getMSlots();
							plShips.at(plShips.size() - 1).hSlots = tempInv.at(i2).sShip.getHSlots();
							plShips.at(plShips.size() - 1).sULSlots = tempInv.at(i2).sShip.getULSlots();
							plShips.at(plShips.size() - 1).sUMSlots = tempInv.at(i2).sShip.getUMSlots();
							plShips.at(plShips.size() - 1).sUHSlots = tempInv.at(i2).sShip.getUHSlots();
							plShips.at(plShips.size() - 1).sEngine = tempInv.at(i2).sShip.sEngine.getID();
							plShips.at(plShips.size() - 1).sPSystem = tempInv.at(i2).sShip.sPSystem.getID();
							plShips.at(plShips.size() - 1).sCPU = tempInv.at(i2).sShip.sCPU.getID();
							plShips.at(plShips.size() - 1).sRAM = tempInv.at(i2).sShip.sRAM.getID();
							plShips.at(plShips.size() - 1).sSPSystem = tempInv.at(i2).sShip.sSPSystem.getID();
							plShips.at(plShips.size() - 1).sSMatrix = tempInv.at(i2).sShip.sSMatrix.getID();
							plShips.at(plShips.size() - 1).sWFGen = tempInv.at(i2).sShip.sWFGen.getID();
							plShips.at(plShips.size() - 1).sAPlating = tempInv.at(i2).sShip.sAPlating.getID();
							plShips.at(plShips.size() - 1).sHSStruct = tempInv.at(i2).sShip.sHSStruct.getID();
							plShips.at(plShips.size() - 1).sWTSystem = tempInv.at(i2).sShip.sWTSystem.getID();
		...
		//Get Slot Vector Data
		for (i = 1; i <= 3; i++)
		{
			if (mPlayer.ship.getSVSize(i) > 0)
			{
				switch (i)
				{
				case 1:
					...
					for (i2 = 0; i2 < mPlayer.ship.getSVSize(i); i2++)
					{
