...
void Map::setPOS(Player& pTeam, NPC& hTeam)
{
	for (i = 0; i < pTeam.getNOWingmen() + 1; i++) //Get number of wingmen then add 1 for player
	{
		...
		else
		{
			...
			pTeam.eWingmen.at(i - 1).ship.setPos(getIRange(x - xR, x + xR), getIRange(y - yR, y + yR), getIRange(z - zR, z + zR)); //Apply a small variation in origination coordinates so that allied/enemy ships are placed near by each other but somewhat randomized
		}
	}
	...
	for (i = 0; i < hTeam.getNOWingmen() + 1; i++) //Get number of wingmen and add 1 for main hostile
	{
		...
		else
		{
			...
			hTeam.eWingmen.at(i - 1).ship.setPos(getIRange(x - xR, x + xR), getIRange(y - yR, y + yR), getIRange(z - zR, z + zR)); //Apply a small variation in origination coordinates so that allied/enemy ships are placed near by each other but somewhat randomized
		}
	}
}
...
{
	encRand1 = getIRange(1, 20);
	encRand2 = getIRange(1, 100);
