						...
						if ((!mPlayer.eWingmen.at(i2).ship.isDestroyed()) && (!mPlayer.eWingmen.at(i2).ship.isDisabled()) && (mPlayer.eWingmen.at(i2).getStatus() == "In Battle")) //If this wingman is not destroyed and is not disabled
						{
							...
							else if ((pBAShields <= 0) && (hostiles > 0))
							{
								if (aBPDTarget == -1)
								{
									while(aBPDTarget == -1)
									{
										i3 = getIRange(0,gPlanet->pDef.size()-1);
...
void battleProc::initHR1(NPC& mNPC)
{
	...
	{
		//NPC init
		rCode = getIRange(1,7); //Race
		sCode = getIRange(1,5); //Ship
		rkCode = 1; //Rank
		...
		if (i == 0)
		{
			...
			if (banks != 0)
			{
				iBanks = getIRange(1,banks); //set a random number between 1 and max number of l. banks for use
			...
			if (turrets != 0)
			{
				iTurrets = getIRange(1,turrets); //set a random number between 1 and max number of m. turrets for use

				for (i2 = 0; i2 <= iTurrets - 1; i2++)
				{
					wCode = getIRange(32,36);
			...
			if (bombs != 0)
			{
				iBombs = getIRange(1,bombs); //set a random number between 1 and max number of b. holds for use

				for (i2 = 0; i2<= iBombs - 1; i2++)
				{

					wCode = getIRange(103,107);
...
void battleProc::initHR2(NPC& mNPC)
{
	for (i = 0; i <= hostiles - 1; i++)
	{
		//NPC init
		rCode = getIRange(1, 7); //Race
		sCode = getIRange(1, 10); //Ship
		rkCode = getIRange(1, 2); //Rank
...
void battleProc::initHR5(NPC& mNPC)
{
	for (i = 0; i <= hostiles - 1; i++)
	{
		//NPC init
		rCode = getIRange(1,7); //Race
		sCode = getIRange(1,16); //Ship
		rkCode = getIRange(1,5); //Rank
...
void battleProc::initHR6(NPC& mNPC)
{
	for (i = 0; i <= hostiles - 1; i++)
	{
		//NPC init
		rCode = getIRange(1,7); //Race
		sCode = getIRange(1,21); //Ship
		rkCode = getIRange(1,6); //Rank
...
void battleProc::initHR7(NPC& mNPC)
{
	for (i = 0; i <= hostiles - 1; i++)
	{
		//NPC init
		rCode = getIRange(1,7); //Race
		sCode = getIRange(1,26); //Ship
		rkCode = getIRange(1, 7); //Rank
...
void battleProc::initHR8(NPC& mNPC)
{
	for (i = 0; i <= hostiles - 1; i++)
	{
		//NPC init
		rCode = getIRange(1,7); //Race
		sCode = getIRange(1,35); //Ship
		rkCode = getIRange(1,8); //Rank
...
void battleProc::initHR9(Player& mPlayer, NPC& mNPC)
{
	for (i = 0; i <= hostiles - 1; i++)
	{
		...
		{
			sCode = getIRange(1,43); //Ship
		}

		else
		{
			sCode = getIRange(1,35);
		}

			rkCode = getIRange(1,9); //Rank
...
void battleProc::aiHTasks(NPC& cNAttacker, Player& mPlayer)
{
	...
	else if ((bAITask1 >= 11) && (bAITask1 <=20)) //Hail
	{
		if (hTarget == 0)
		{
			...
			if ((bHPPerc1 > 75.00) && ((bHPPerc2 <= 35.00) && (bHPPerc2 > 0.00))) //Demand surrender if conditions are met
			{
				...
				if (mp.bNHail1())
				{
					if (mp.bNHDemand())
					{
						...
						{
							bAITask3 = getIRange(1, 9);

							for (i2 = 0; i2 < bAITask3; i2++)
							{
								while (false)
								{
									bAITask4 = getIRange(0, 8);

									if (rArray[bAITask4] == 0)
									{
										rArray[bAITask4] = getIRange(100, 10000);
										break;
									}
								}
							}
			...
			{
				bAITask3 = getIRange(1, 10);
				bAITask4 = getIRange(1, 10);
				bAITask5 = getIRange(1, 10);
	...
	else if ((bAITask1 >= 31) && (bAITask1 <= 40)) //Retreat
	{
		...
		{
			bAITask3 = getIRange(1, 30);
			bAITask4 = getIRange(1, 25);
			bAITask5 = getIRange(1, 40);
