...
void Damage::processWQueue(bEntity& bAttacker,bEntity& bTarget)
{
	...
	for (i = 0; i < weaponQueue.size(); i++)
	{
		...
		{
			vDamage += getFRange(dLow, dHigh);
		}

		else if (wDType != "null")
		{
			sDamage = getFRange(dLow, dHigh); //Needs explaining; sDamage becomes the vehicle of vDamage in processSDamage, we take the damage done by the current weapon through to that function for processing and NOT vDamage because vD could have damage from other weapons
...
void Damage::processWQueue_PB(bEntity& bAttacker,bEntity& bTarget, bool pAPDef, bool pAPShd, bool pADef)
{
	...
	for (i = 0; i < weaponQueue.size(); i++)
	{
		...
		else if (weaponQueue.at(i).getHPWType() == "bomb")
		{
			...
			dBCollateral = (bAttacker.entity->ship.bHolds.at(weaponQueue.at(i).getHPSlot()).hpWeapon.getWeight() * getIRange(7850, 16450)) / 2; //Calculate collateral damage caused by weight of bomb
